...
predictors.self  <-  data.frame(
                             stay_choice = character(), 
                             outcome = double(), 
                             outcome_diff = double()
)
...
for (n in 1:num_sub){
  ...
  mf_index[n]  <-  round(sum_reg[["estimate"]][5], digits = 3) #MF_index
  p.value_mf[n]  <-  round(sum_reg[["p.value"]][5], digits = 3)
...
for (n in 1:num_sub){
  sub <- subject_id[n]
  #dir other
  dir_other_O <- paste(pre_other,n,"/data.mat", sep ="")
  ...
  mf_index_O[n] <-  round(sum_reg_O[["estimate"]][5], digits = 3)        #MF_index
  p.value_mf_O[n] <-  round(sum_reg_O[["p.value"]][5], digits = 3)
   #making a data frame of mb/mf indexes and ...
...
my_plot <- GGally::ggscatmat(index_data_O, columns = c("accuracy", "mb_index", "mf_index" )) + 
  theme_minimal()
#save plot
ggsave('E:/uni/statistics/Learning-in-self-other-Decision-making/plot/Association_other.png',
       my_plot, device = "png", width = 10, height = 7, dpi = 300)
#Pearson’s correlation 
index_data_O %>%
  dplyr::select(accuracy, mb_index, mf_index) %>%
...
#pool---------------------------------------------------------------------------
#data
max_dat <- data.frame(id = c(index_data$id, index_data_O$id),
                      max_rew = c(index_data$max_reward, index_data_O$max_reward),
                      accuracy = c(index_data$accuracy, index_data_O$accuracy),
                      performance = c(index_data$performance, index_data_O$performance)
)
#plot
my_plot <- ggplot2::ggplot(index_data_O, aes(mb_index, accuracy)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "mb index", y = "Accuracy") + #Maximum Attainable Reward
  theme_minimal()
#save plot
ggsave('E:/uni/statistics/Learning-in-self-other-Decision-making/plot/mb_acc_O.png',
       my_plot, device = "png", width = 10, height = 7, dpi = 300)
#regression
max_lm <- lm(performance ~ mb_index, data = index_data) 
summary(max_lm)
#Pearson’s correlation 
max_dat %>%
  dplyr::select(accuracy, max_rew) %>%
  correlation::correlation(method = "percentage")
...
```{r}
#GLM
perf_lm <- lm(performance ~ mb_index, data = index_data_O, na.action = na.exclude)
...
bar <- ggplot(bar_tib, aes(strategy, index, fill = group))
#plot
...
m1 <- lm( perf ~ mb_mf * group, data=index_tib ) #multiple regression
m1 <- aov(perf ~ mb_mf * group, data=index_tib)  #ANCOVA
summary(m1)
broom::glance(m1)
...
ggplot(index_tib, aes(x = mb_mf, col = group, fill = group)) + 
...
ggplot2::ggplot(index_tib, aes(mb_mf, perf, colour = group)) +
  geom_point() +
...
reg_out_lm <- lm(perf ~ max_reward * group, data = reg_out_data)
