...
for (n in 1:num_sub){
  #dir self
  dir_self <- paste(pre_self,n,"/data.mat", sep ="") 
...
for (n in 1:num_sub){
  #dir other
  dir_self <- paste(pre_other,n,"/data.mat", sep ="") 
  subject <- readMat(dir_self)
  #choice other
  ch <- subject$data[, , 1]$choice
  ind_choice <- which(ch!=0)
...
stay.data.frame <- stay.data.frame.self
...
group_data <- my_data %>% 
  dplyr::group_by(id,outcome_valence, beneficiary)%>%
  dplyr::summarize(
...
# plot the mean and a 95% confidence interval of stay probability scores split by the stimulus (x-axis).
ggplot2::ggplot(my_data, aes(x = outcome_valence, y = stay_prob)) +
  geom_point(colour = "#2C5577", alpha = 0.7, position = position_jitter(width = 0.1)) +
  stat_summary(fun.data = "mean_cl_normal", geom = "pointrange") +
  labs(x = "outcome valence", y = "stay probability") +
